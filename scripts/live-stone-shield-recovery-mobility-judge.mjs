 ...
 * keys: STONE SHIELD RECOVERY MOBILITY MOBILE DESKTOP CHROMIUM
 * purpose: observe real Stone Shield recovery locomotion with identical browser trace across variants
...
try{
  for(const sc of scenarios){
    ...
    try{
      ...
      await page.goto(`${URL}?offline=1&stone-shield-recovery-judge=${Date.now()}`,{waitUntil:'domcontentloaded',timeout:45000});
      ...
      await page.evaluate(async()=>{
        ...
        const def=window.KeloAbilities.registry.getByKey('stone_shield');
        if(!def) throw new Error('STONE_SHIELD_DEFINITION_MISSING');
        if(typeof STATE==='undefined'||!STATE) throw new Error('STATE_MISSING');
        const stone=window.KeloAbilities.stones.createAbilityStone('stone_shield','Common',{source:'stone-shield-recovery-judge'});
      ...
      const result=await page.evaluate(async()=>{
        ...
        const slot=slots.findIndex(s=>s?.definition?.key==='stone_shield');
        if(slot<0) throw new Error('STONE_SHIELD_SLOT_MISSING_AFTER_TEST_EQUIP');
        window.__stoneShieldMoveHook=window.KeloInput.after('stone-shield-recovery:move-left',ctx=>{if(ctx?.input){ctx.input.normX=-1;ctx.input.normY=0;}},9999);
        localPlayer.x=2860;localPlayer.y=700;localPlayer.vx=0;localPlayer.vy=0;
        const ok=window.KeloPvPWorld.quickCastSlot(slot);
        if(ok===false) throw new Error('STONE_SHIELD_CAST_REJECTED');
        const deadline=performance.now()+2500;
        while(performance.now()<deadline){
          const a=window.KELO_PVP_CAST_MOVEMENT_AUDIT;
          if(a?.active&&a.phase==='recovery'&&a.abilityKey==='stone_shield') break;
          await new Promise(requestAnimationFrame);
        }
        const audit=window.KELO_PVP_CAST_MOVEMENT_AUDIT;
        if(!audit?.active||audit.phase!=='recovery'||audit.abilityKey!=='stone_shield') throw new Error('STONE_SHIELD_RECOVERY_NOT_REACHED');
        ...
        window.__stoneShieldMoveHook?.off?.();
      ...
      if(!(result.scale>=.54&&result.scale<=.81)) throw new Error(`${sc.name}:BAD_SCALE:${result.scale}`);
      if(!(result.recoverySampleCount>=3)) throw new Error(`${sc.name}:TOO_FEW_RECOVERY_SAMPLES:${result.recoverySampleCount}`);
      if(!(result.motion>5)) throw new Error(`${sc.name}:NO_MEANINGFUL_MOTION:${result.motion}`);
      ...
      out.push(result);
    } catch(error){failure={scenario:sc.name,message:String(error?.message||error),pageErrors:errors};throw error;}
    finally {await context.close();}
  }
} catch(error){failure=failure||{scenario:'unknown',message:String(error?.message||error)};}
finally {await browser.close();}
fs.mkdirSync('audit-artifacts',{recursive:true});
fs.writeFileSync('audit-artifacts/stone-shield-recovery-live.json',JSON.stringify({ok:!failure,scenarios:out,failure},null,2));
console.log(JSON.stringify({ok:!failure,scenarios:out,failure},null,2));
if(failure) throw new Error(`LIVE_STONE_SHIELD_RECOVERY_MOBILITY_FAIL:${failure.scenario}:${failure.message}`);
console.log('LIVE_STONE_SHIELD_RECOVERY_MOBILITY_JUDGE_OK');

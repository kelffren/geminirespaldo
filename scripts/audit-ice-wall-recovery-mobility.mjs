 ...
 * keys: ICE WALL RECOVERY MOBILITY REVERSAL 60HZ 90HZ 120HZ A B
 * purpose: same-trace baseline/candidate evaluation for phase-aware Ice Wall recovery locomotion
...
function run(scale,hz){
  ...
  if(!wind.every(x=>Math.abs(x.scale-BASELINE)<1e-9)||!active.every(x=>Math.abs(x.scale-BASELINE)<1e-9)) throw new Error('ICE_WALL_COMMITMENT_CHANGED');
...
for(let i=0;i<3;i++){
  if(a[i].recoveryPx<=base[i].recoveryPx+7) throw new Error('CANDIDATE_A_GAIN_TOO_SMALL');
  if(b[i].recoveryPx<=a[i].recoveryPx+2) throw new Error('CANDIDATE_B_REFINEMENT_TOO_SMALL');
